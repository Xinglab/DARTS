...
def read_data_batch_encode(n_epoch=500, verbose=True):
...
def read_data_batch_roadmap(n_epoch=500, verbose=True):
...
def train_dnn_classifier():
	...
	try:
		clf.model.load_weights('.keras_model.h5')
	except:
		pass
	# read each dataset iteratively
	gen_encode = read_data_batch_encode()
	gen_roadmap = read_data_batch_roadmap()
	index = 0
	best_test_auc = 0
	while True:
		index += 1
		try:
			Y_batch1, X_batch1, msg1 = gen_roadmap.next()
			Y_batch2, X_batch2, msg2 = gen_encode.next()
		...
		if auroc>best_test_auc:
			best_test_auc = auroc
			clf.model.save('best_testing_model.h5')
		if not index%10:
...
if __name__ == '__main__':
	...
	logging.basicConfig(level=logging.DEBUG,
				...
				filename='log.dnn.'+ '-'.join(str(datetime.datetime.now()).replace(':','-').split())+'.txt' ,
				filemode='w')
